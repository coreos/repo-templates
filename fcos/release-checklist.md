...
Edit the issue title to include today's date. Once the pipeline spits out the new version ID, you can append it to the title e.g. `(31.20191117.{{ z_version }}.0)`.

# Pre-release

## Promote {{ source_stream }} changes to {{ stream }}
...
- `git fetch upstream`
- `git checkout {{ stream }}`
- `git reset --hard upstream/{{ stream }}`
- `/path/to/fedora-coreos-releng-automation/scripts/promote-config.sh {{ source_stream }}`
- Open PR against the `{{ stream }}` branch on https://github.com/coreos/fedora-coreos-config
...
- Wait for the jobs to finish and succeed
    ...
    - [ ] ppc64le
    - [ ] s390x
- [ ] Edit the issue title to append the new version ID e.g. `(31.20191117.{{ z_version }}.0)`.

## Sanity-check the build

Using the [the build browser for the `{{ stream }}` stream](https://builds.coreos.fedoraproject.org/browser?stream={{ stream }}):

- Verify that the parent commit and version match the previous `{{ stream }}` release (in the future, we'll want to integrate this check in the release job)
    - [ ] x86_64
    - [ ] aarch64
    - [ ] ppc64le
    - [ ] s390x
...
- [ ] Run the [release job](https://jenkins-fedora-coreos-pipeline.apps.ocp.fedoraproject.org/job/release/), filling in for parameters `{{ stream }}` and the new version ID
...
```
fedora-coreos-stream-generator -releases=https://fcos-builds.s3.amazonaws.com/prod/streams/{{ stream }}/releases.json  -output-file=streams/{{ stream }}.json -pretty-print
```

- Add a rollout.  For example, for a 48-hour rollout starting at 10 AM ET the same day, run:

```
./rollout.py add {{ stream }} <version> "10 am ET today" 48
```
...
- [ ] Verify the new version shows up on [the download page](https://getfedora.org/en/coreos/download?stream={{ stream }})
- Verify the incoming edges are showing up in the update graph.
    - [ ] [x86_64](https://builds.coreos.fedoraproject.org/graph?stream={{ stream }}&basearch=x86_64)
    - [ ] [aarch64](https://builds.coreos.fedoraproject.org/graph?stream={{ stream }}&basearch=aarch64)
    - [ ] [ppc64le](https://builds.coreos.fedoraproject.org/graph?stream={{ stream }}&basearch=ppc64le)
    - [ ] [s390x](https://builds.coreos.fedoraproject.org/graph?stream={{ stream }}&basearch=s390x)

<details>
  <summary>Update graph manual check</summary>

```
curl -H 'Accept: application/json' 'https://updates.coreos.fedoraproject.org/v1/graph?basearch=x86_64&stream={{ stream }}&rollout_wariness=0'
curl -H 'Accept: application/json' 'https://updates.coreos.fedoraproject.org/v1/graph?basearch=aarch64&stream={{ stream }}&rollout_wariness=0'
curl -H 'Accept: application/json' 'https://updates.coreos.fedoraproject.org/v1/graph?basearch=ppc64le&stream={{ stream }}&rollout_wariness=0'
curl -H 'Accept: application/json' 'https://updates.coreos.fedoraproject.org/v1/graph?basearch=s390x&stream={{ stream }}&rollout_wariness=0'
```
